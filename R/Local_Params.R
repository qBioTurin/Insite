#' Local_Params class
#'
#' S4 class storing local parameters used for simulation updates at a specific
#' time step.
#'
#' @slot phenotypes_local A list of local phenotypes.
#' @slot a_local A list of local birth parameters.
#' @slot b_local A list of local death parameters.
#' @slot mu_local A list of local mutation parameters.
#' @slot rel_freq_local A list of local relative frequencies.
#' @slot Delta Numeric time increment used in the local update.
#'
#' @name Local_Params-class
#' @rdname Local_Params-class
...
#' Compute local simulation parameters
#'
#' Computes local birth, death, mutation, and relative-frequency parameters for
#' the populations present at a given simulation step.
#'
#' @param Parameters A [`Parameters-class`] object containing global simulation
#'   parameters.
#' @param list_of_pops_with_size_nmut A list of population objects with
#'   associated sizes and mutation information.
#' @param count Numeric index of the current checkpoint.
#' @param time_provv Current simulation time.
#' @param epsilon Numeric scaling factor used in the computation of `Delta`.
#'
#' @return A [`Local_Params-class`] object.
#'
#' @noRd
...
setMethod("get_local_params",
          ...
          function(Parameters,list_of_pops_with_size_nmut,count,time_provv,epsilon){
            ...
            if(time_provv+Delta*Parameters@av_lifespan>Parameters@print_time[count+1]){
...
#' Compute offspring count probabilities
#'
#' Computes the probability distribution used for the local branching process
#' associated with a [`Local_Params-class`] object.
#'
#' @param local_params A [`Local_Params-class`] object.
#'
#' @return A list with two elements:
#' \describe{
#'   \item{p}{A list of probability vectors.}
#'   \item{phenotypes_local}{The local phenotypes associated with the probabilities.}
#' }
#'
#' @noRd
