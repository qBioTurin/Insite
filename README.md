...
## Repository of _A New Cancer Progression Model: From synthetic tumors to real data and back_
...
- `PaperFigures/`: R scripts dedicated to generating the specific figures (e.g., `Fig3.R`, `Fig5.R`).
  - _Note:_ These scripts are designed for interactive use (e.g., within RStudio). They will render plots directly to your active graphics device rather than saving files to disk.

#### R Package (`R/`, `NAMESPACE`, `DESCRIPTION`, `man`)

All the necessary for the R package `Insite` to work.
...
2. Run any script in `scripts/PaperFigures/`. The plots will appear in the _Plots_ panel. Each script is named after a figure.
